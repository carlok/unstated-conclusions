...
Recovering it needs a re-run, which is not free (~4 days for Mathlib). Before
spending that: confirm this is the field dichotomy repair actually turns on, and
I will widen the capture to the whole error block and re-sweep. Cheaper
alternative if you only need it for a subset — give me a row list and I will
re-compile just those and fill `blamed` in.
